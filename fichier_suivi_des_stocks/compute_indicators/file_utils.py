...
def process_etat_stock_npsp(
    ...
    Ajoute la date du rapport et le programme, puis sélectionne les colonnes
    pertinentes.

    Args:
        df_etat_stock_npsp (pd.DataFrame): Le DataFrame contenant les données
            de l'état des stocks NPSP.
        date_report (str): La date du rapport au format 'YYYY-MM-DD'.
        programme (str): Le programme associé aux données.

    Returns:
        pd.DataFrame: Le DataFrame traité avec les colonnes renommées,
        la date du rapport et le programme ajoutés.
    """
    column_mapping = {
        ...
        r"Stock Inventaire BOUAKE": "stock_theorique_bke",
        r"Stock Inventaire ABIDJAN": "stock_theorique_abj",
        r"Stock Inventaire CENTRALE": "stock_theorique_central",
        r"Stock theorique fin": "stock_theorique_fin_mois",
...
def _get_code_and_date_concate(row: pd.Series) -> str | float:
    try:
...
def _process_pa_version(pa_version: str) -> list:
...
def adapt_programme_file_to_template(
    programme_wb: pyxl.Workbook, template_wb: pyxl.Workbook, fp_etat_mensuel: PosixPath
) -> PosixPath:
    ...
    Args:
        programme_wb: pyxl.Workbook: Le classeur du fichier programme à adapter.
        template_wb: pyxl.Workbook: Le classeur du template de référence.
